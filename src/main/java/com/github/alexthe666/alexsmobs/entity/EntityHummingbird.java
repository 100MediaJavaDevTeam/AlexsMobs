...
public class EntityHummingbird extends AnimalEntity {
    ...
    private static final DataParameter<Boolean> FLYING = EntityDataManager.defineId(EntityHummingbird.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Integer> VARIANT = EntityDataManager.defineId(EntityHummingbird.class, DataSerializers.INT);
    private static final DataParameter<Integer> CROPS_POLLINATED = EntityDataManager.defineId(EntityHummingbird.class, DataSerializers.INT);
    private static final DataParameter<Optional<BlockPos>> FEEDER_POS = EntityDataManager.defineId(EntityHummingbird.class, DataSerializers.OPTIONAL_BLOCK_POS);
    ...
    public static AttributeModifierMap.MutableAttribute bakeAttributes() {
        return MonsterEntity.createMonsterAttributes().add(Attributes.MAX_HEALTH, 4.0D).add(Attributes.FLYING_SPEED, 7F).add(Attributes.ATTACK_DAMAGE, 0.0D).add(Attributes.MOVEMENT_SPEED, 0.45F);
    }
    ...
    public boolean canBlockBeSeen(BlockPos pos) {
        ...
        RayTraceResult result = this.level.clip(new RayTraceContext(new Vector3d(this.getX(), this.getY() + (double) this.getEyeHeight(), this.getZ()), new Vector3d(x, y, z), RayTraceContext.BlockMode.COLLIDER, RayTraceContext.FluidMode.NONE, this));
    ...
    private class AIUseFeeder extends Goal {
        ...
        public boolean canUse() {
           ...
           if(runCooldown > 0){
               ...
               if(feedPos != null && isValidFeeder(level.getBlockState(feedPos))){
                   ...
                   List<BlockPos> beacons = getNearbyFeeders(EntityHummingbird.this.blockPosition(), (ServerWorld) level, 64);
        ...
        public void tick(){
            if(localFeeder != null && isValidFeeder(level.getBlockState(localFeeder))){
                ...
                Vector3d vec = Vector3d.upFromBottomCenterOf(localFeeder, 0.1F);
                double dist = MathHelper.sqrt(EntityHummingbird.this.distanceToSqr(vec));

...
public class EntityWarpedMosco extends MonsterEntity implements IAnimatedEntity {
    ...
    private static final DataParameter<Boolean> FLYING = EntityDataManager.defineId(EntityWarpedMosco.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean> HAND_SIDE = EntityDataManager.defineId(EntityWarpedMosco.class, DataSerializers.BOOLEAN);
    ...
    public static AttributeModifierMap.MutableAttribute bakeAttributes() {
        return MonsterEntity.createMonsterAttributes().add(Attributes.MAX_HEALTH, 100D).add(Attributes.FOLLOW_RANGE, 128.0D).add(Attributes.ATTACK_DAMAGE, 10.0D).add(Attributes.ARMOR, 10D).add(Attributes.KNOCKBACK_RESISTANCE, 1D).add(Attributes.ARMOR_TOUGHNESS, 2D).add(Attributes.MOVEMENT_SPEED, 0.3D);
    }
    ...
    public void tick() {
        ...
        ITag<Block> transformMatches = BlockTags.getAllTags().getTag(AMTagRegistry.WARPED_MOSCO_BREAKABLES);
    ...
    public void spawnGroundEffects() {
        ...
        for (int i = 0; i < 4; i++) {
            for (int i1 = 0; i1 < 20 + random.nextInt(12); i1++) {
                ...
                BlockPos ground = getMoscoGround(new BlockPos(MathHelper.floor(this.getX() + extraX), MathHelper.floor(this.getY() + extraY) - 1, MathHelper.floor(this.getZ() + extraZ)));
    ...
    public void positionRider(Entity passenger) {
        ...
        if (hasPassenger(passenger)) {
            ...
            if ((tick - 10) % 4 == 0) {
                this.addEffect(new EffectInstance(Effects.REGENERATION, 100, 1));
    ...
    private class AIWalkIdle extends Goal {
        ...
        protected Vector3d getPosition() {
            Vector3d vector3d = mosco.position();
            ...
            if (flightTarget) {
                ...
                return RandomPositionGenerator.getPos(this.mosco, 20, 7);
            }
        }
    ...
    private class AttackGoal extends Goal {
        ...
        public void tick() {
            ...
            if (EntityWarpedMosco.this.getTarget() != null) {
                ...
                if (EntityWarpedMosco.this.isFlying() || ranged || EntityWarpedMosco.this.distanceTo(target) > 12 && !EntityWarpedMosco.this.isTargetBlocked(target.position().add(0, target.getBbHeight() * 0.6F, 0))) {
                    ...
                    if (ranged) {
                        if (farTarget == null || EntityWarpedMosco.this.distanceToSqr(Vector3d.atCenterOf(farTarget)) < 9) {
        ...
        public BlockPos getAvoidTarget(LivingEntity target) {
            ...
            if (EntityWarpedMosco.this.distanceToSqr(Vector3d.atCenterOf(ground)) > 30) {
                if (!EntityWarpedMosco.this.isTargetBlocked(Vector3d.atCenterOf(ground)) && EntityWarpedMosco.this.distanceToSqr(Vector3d.atCenterOf(ground)) > 6) {
                    return ground;
                }
            }

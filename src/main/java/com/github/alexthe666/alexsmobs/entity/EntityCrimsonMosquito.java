...
public class EntityCrimsonMosquito extends MonsterEntity {
    ...
    private static final DataParameter<Boolean> FLYING = EntityDataManager.defineId(EntityCrimsonMosquito.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean> SHOOTING = EntityDataManager.defineId(EntityCrimsonMosquito.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Integer> BLOOD_LEVEL = EntityDataManager.defineId(EntityCrimsonMosquito.class, DataSerializers.INT);
    private static final DataParameter<Boolean> SHRINKING = EntityDataManager.defineId(EntityCrimsonMosquito.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean> FROM_FLY = EntityDataManager.defineId(EntityCrimsonMosquito.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Float> MOSQUITO_SCALE = EntityDataManager.defineId(EntityCrimsonMosquito.class, DataSerializers.FLOAT);
    private static final DataParameter<Boolean> SICK = EntityDataManager.defineId(EntityCrimsonMosquito.class, DataSerializers.BOOLEAN);
    ...
    public static AttributeModifierMap.MutableAttribute bakeAttributes() {
        return MonsterEntity.createMonsterAttributes().add(Attributes.MAX_HEALTH, 10.0D).add(Attributes.FOLLOW_RANGE, 32.0D).add(Attributes.ARMOR, 0.0D).add(Attributes.ATTACK_DAMAGE, 5.0D).add(Attributes.MOVEMENT_SPEED, 0.25F);
    }
    ...
    public static boolean canMosquitoSpawn(EntityType<? extends MobEntity> typeIn, IServerWorld worldIn, SpawnReason reason, BlockPos pos, Random randomIn) {
        ...
        return reason == SpawnReason.SPAWNER || spawnBlock && worldIn.getBlockState(blockpos).isValidSpawn(worldIn, blockpos, typeIn)  && isDarkEnoughToSpawn(worldIn, pos, randomIn) && checkMobSpawnRules(AMEntityRegistry.CRIMSON_MOSQUITO, worldIn, reason, pos, randomIn);
    }
                        ...
                        if(mount.hurt(DamageSource.mobAttack(this), mungus ? 7F : 2.0F)){
                            ...
                            if(sick || mungus){
                                if(!this.isSick() && !level.isClientSide){
                                    for(ServerPlayerEntity serverplayerentity : this.level.getEntitiesOfClass(ServerPlayerEntity.class, this.getBoundingBox().inflate(40.0D, 25.0D, 40.0D))) {
        ...
        if(this.isSick()){
            ...
            if(sickTicks > 100){
                ...
                if(sickTicks > 160){
                    ...
                    if(!level.isClientSide){
                        mosco.finalizeSpawn((IServerWorld)level, level.getCurrentDifficultyAt(this.blockPosition()), SpawnReason.CONVERSION, null, null);
                    }
    ...
    static class RandomFlyGoal extends Goal {
        ...
        public boolean canContinueToUse() {
            return target != null && parentEntity.isFlying() && parentEntity.distanceToSqr(Vector3d.atCenterOf(target)) > 2.4D && parentEntity.getMoveControl().hasWanted() && !parentEntity.horizontalCollision;
        }

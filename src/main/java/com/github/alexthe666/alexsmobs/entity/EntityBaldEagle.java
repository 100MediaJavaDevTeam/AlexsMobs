...
public class EntityBaldEagle extends TameableEntity implements IFollower {
    ...
    private static final DataParameter<Boolean> FLYING = EntityDataManager.defineId(EntityBaldEagle.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean> TACKLING = EntityDataManager.defineId(EntityBaldEagle.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean> HAS_CAP = EntityDataManager.defineId(EntityBaldEagle.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Integer> ATTACK_TICK = EntityDataManager.defineId(EntityBaldEagle.class, DataSerializers.INT);
    private static final DataParameter<Integer> COMMAND = EntityDataManager.defineId(EntityBaldEagle.class, DataSerializers.INT);
    private static final DataParameter<Boolean> SITTING = EntityDataManager.defineId(EntityBaldEagle.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean> LAUNCHED = EntityDataManager.defineId(EntityBaldEagle.class, DataSerializers.BOOLEAN);
    ...
    public static AttributeModifierMap.MutableAttribute bakeAttributes() {
        return MonsterEntity.createMonsterAttributes().add(Attributes.MAX_HEALTH, 16.0D).add(Attributes.FOLLOW_RANGE, 32.0D).add(Attributes.ATTACK_DAMAGE, 5.0D).add(Attributes.MOVEMENT_SPEED, 0.3F);
    }
    ...
    public ActionResultType mobInteract(PlayerEntity player, Hand hand) {
        ...
        if (item.is(ItemTags.FISHES) && this.getHealth() < this.getMaxHealth()) {
            ...
            if (!this.isBaby() && item == AMItemRegistry.FALCONRY_HOOD) {
                ...
                this.setCap(false);
                return ActionResultType.SUCCESS;
            }else if (!this.isBaby() && getRidingEagles(player) <= 0 && (player.getItemInHand(Hand.MAIN_HAND).getItem() == AMItemRegistry.FALCONRY_GLOVE || player.getItemInHand(Hand.OFF_HAND).getItem() == AMItemRegistry.FALCONRY_GLOVE)) {
                boardingCooldown = 30;
        ...
        if (this.isPassenger() && (!entity.isAlive() || !this.isAlive())) {
            ...
            if (this.isPassenger()) {
                ...
                if (mount instanceof PlayerEntity) {
                    float yawAdd = 0;
                    if (((PlayerEntity) mount).getItemInHand(Hand.MAIN_HAND).getItem() == AMItemRegistry.FALCONRY_GLOVE) {
                        yawAdd = ((PlayerEntity) mount).getMainArm() == HandSide.LEFT ? 135 : -135;
                    } else if (((PlayerEntity) mount).getItemInHand(Hand.OFF_HAND).getItem() == AMItemRegistry.FALCONRY_GLOVE) {
                        yawAdd = ((PlayerEntity) mount).getMainArm() == HandSide.LEFT ? -135 : 135;
                    } else {
                        this.setCommand(2);
                        this.setOrderedToSit(true);
                    ...
                    this.yBodyRot = MathHelper.wrapDegrees(((LivingEntity) mount).yBodyRot + birdYaw);
                    this.yRot = MathHelper.wrapDegrees(((LivingEntity) mount).yRot + birdYaw);
                    this.yHeadRot = MathHelper.wrapDegrees(((LivingEntity) mount).yHeadRot + birdYaw);
                    float radius = 0.6F;
    ...
    private Vector3d getOrbitVec(Vector3d vector3d, float gatheringCircleDist) {
        ...
        if (this.orbitPos != null) {
            Vector3d pos = new Vector3d(orbitPos.getX() + extraX, orbitPos.getY() + random.nextInt(2) - 2, orbitPos.getZ() + extraZ);
    ...
    public void positionRider(Entity passenger) {
        if (this.hasPassenger(passenger)) {
            ...
            double extraY = 0;
            if (passenger instanceof AbstractFishEntity && !passenger.isInWaterOrBubble()) {
                extraY = 0.1F;
            }
    ...
    public void directFromPlayer(float rotationYaw, float rotationPitch, boolean loadChunk, Entity over) {
        ...
        if (!level.isClientSide) {
            ...
            this.setTarget(null);
            if (over == null) {
                List<Entity> list = this.level.getEntities(this, this.getBoundingBox().inflate(3.0D), EntityPredicates.NO_CREATIVE_OR_SPECTATOR);
                Entity closest = null;
                for (Entity e : list) {
        ...
        if (over != null && !this.isAlliedTo(over) && over != owner && canFalconryAttack(over)) {
            if (tackleCapCooldown == 0 && this.distanceTo(over) <= over.getBbWidth() + 4D) {
                ...
                if (this.distanceTo(over) <= over.getBbWidth() + 2D) {
                    float speedDamage = (float) Math.ceil(MathHelper.clamp(this.getDeltaMovement().length() + 0.2, 0, 1.2D) * 3.333);
    ...
    public void killed(ServerWorld world, LivingEntity entity) {
        if (this.isLaunched() && this.hasCap() && this.isTame() && this.getOwner() != null) {
            if (this.getOwner() instanceof ServerPlayerEntity && this.distanceTo(this.getOwner()) >= 100) {
    ...
    public boolean hurt(DamageSource source, float amount) {
        if (this.isInvulnerableTo(source)) {
            ...
            if (entity != null && this.isTame() && !(entity instanceof PlayerEntity) && !(entity instanceof AbstractArrowEntity) && this.isLaunched()) {
    ...
    private class AITackle extends Goal {
        ...
        public void tick() {
            ...
            boolean smallPrey = target != null && target.getBbHeight() < 1F && target.getBbWidth() < 0.7F && !(target instanceof EntityBaldEagle) || target instanceof AbstractFishEntity;
            if (eagle.orbitPos != null && circleTime < maxCircleTime) {
                ...
                Vector3d vec = eagle.getOrbitVec(Vector3d.ZERO, 4 + random.nextInt(2));
                if (vec != null) {

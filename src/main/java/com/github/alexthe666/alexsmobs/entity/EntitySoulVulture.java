...
public class EntitySoulVulture extends MonsterEntity implements IFlyingAnimal {
    ...
    private static final DataParameter<Boolean> FLYING = EntityDataManager.defineId(EntitySoulVulture.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean> TACKLING = EntityDataManager.defineId(EntitySoulVulture.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Optional<BlockPos>> PERCH_POS = EntityDataManager.defineId(EntitySoulVulture.class, DataSerializers.OPTIONAL_BLOCK_POS);
    private static final DataParameter<Integer> SOUL_LEVEL = EntityDataManager.defineId(EntitySoulVulture.class, DataSerializers.INT);
    ...
    public static boolean canVultureSpawn(EntityType<? extends MobEntity> typeIn, IServerWorld worldIn, SpawnReason reason, BlockPos pos, Random randomIn) {
        ...
        return reason == SpawnReason.SPAWNER || spawnBlock && checkMobSpawnRules(AMEntityRegistry.SOUL_VULTURE, worldIn, reason, pos, randomIn);
    }
    ...
    public static AttributeModifierMap.MutableAttribute bakeAttributes() {
        return MonsterEntity.createMonsterAttributes().add(Attributes.MAX_HEALTH, 12.0D).add(Attributes.FOLLOW_RANGE, 18.0D).add(Attributes.ATTACK_DAMAGE, 4.0D).add(Attributes.MOVEMENT_SPEED, 0.25F);
    }
    ...
    private boolean shouldLeavePerch(BlockPos perchPos) {
        return this.distanceToSqr(Vector3d.atCenterOf(perchPos)) > 13 || landingCooldown == 0;
    }
            ...
            if (encircle != null) {
                ...
                if(circlingTime > 360){
                    ...
                    if(vulture.verticalCollision || this.vulture.distanceToSqr(encircle.getX() + 0.5D, encircle.getY() + 1.1D, encircle.getZ() + 0.5D) < 1D){
                        ...
                        vulture.setDeltaMovement(Vector3d.ZERO);
    ...
    private class AIFlyRandom extends Goal {
        ...
        public boolean canUse() {
            ...
            MovementController movementcontroller = this.vulture.getMoveControl();
        ...
        public boolean canContinueToUse() {
            ...
            return target != null && vulture.distanceToSqr(Vector3d.atCenterOf(target)) > 2.4D && vulture.getMoveControl().hasWanted() && !vulture.horizontalCollision;
        }
        ...
        public void tick() {
            ...
            if (target != null) {
                this.vulture.getMoveControl().setWantedPosition(target.getX() + 0.5D, target.getY() + 0.5D, target.getZ() + 0.5D, 1.0D);
                if(vulture.distanceToSqr(Vector3d.atCenterOf(target)) < 2.5F){
                    target = null;
                }
            }
        }
        ...
        public BlockPos getBlockInViewVulture() {
            ...
            if (!vulture.isTargetBlocked(Vector3d.atCenterOf(newPos)) && vulture.distanceToSqr(Vector3d.atCenterOf(newPos)) > 6) {
                return newPos;
            }
            return null;
        }

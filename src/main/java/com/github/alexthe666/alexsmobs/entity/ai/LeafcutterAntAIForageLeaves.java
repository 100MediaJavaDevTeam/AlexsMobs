    ...
    public void tick() {
        ...
        if (ant.getAttachmentFacing() == Direction.UP) {
            ...
            if (logStartPos != null) {
                ...
                Vector3d vec = new Vector3d(logStartPos.getX() + 0.5, ant.getY(), logStartPos.getZ() + 0.5);
        ...
        if (this.isReachedTarget() || ant.blockPosition().above().equals(blockPos)) {
            ant.lookAt(EntityAnchorArgument.Type.EYES, new Vector3d(blockPos.getX() + 0.5D, blockPos.getY(), blockPos.getZ() + 0.5));

...
public class EntitySpectre extends AnimalEntity implements IFlyingAnimal {
    ...
    private static final DataParameter<Integer> CARDINAL_ORDINAL = EntityDataManager.defineId(EntitySpectre.class, DataSerializers.INT);
    ...
    public static AttributeModifierMap.MutableAttribute bakeAttributes() {
        return MonsterEntity.createMonsterAttributes().add(Attributes.MAX_HEALTH, 50.0D).add(Attributes.FOLLOW_RANGE, 64.0D).add(Attributes.ATTACK_DAMAGE, 2.0D).add(Attributes.MOVEMENT_SPEED, 1F);
    }
    ...
    public void tick() {
        ...
        this.yRot = -((float) MathHelper.atan2(vector3d1.x, vector3d1.z)) * (180F / (float) Math.PI);
    ...
    private class FlyGoal extends Goal {
        ...
        public void tick() {
            if (islandCheckTime-- <= 0) {
                islandCheckTime = 20;
                if (circlingTime == 0) {
                    island = this.parentEntity.level.getHeightmapPos(Heightmap.Type.MOTION_BLOCKING_NO_LEAVES, parentEntity.blockPosition()).getY() > 2;
                    if (island) {
            ...
            if (target != null) {
                this.parentEntity.getMoveControl().setWantedPosition(target.getX() + 0.5D, target.getY() + 0.5D, target.getZ() + 0.5D, 1.0D);
                if (parentEntity.distanceToSqr(Vector3d.atCenterOf(target)) < 5.5F) {
                    target = null;
                }
            }
        }
        ...
        public BlockPos getBlockFromDirection() {
            ...
            int height = 0;
            if (level.getHeightmapPos(Heightmap.Type.MOTION_BLOCKING_NO_LEAVES, forwards).getY() < 15) {
                height = 70 + random.nextInt(2);
            } else {
                height = level.getHeightmapPos(Heightmap.Type.MOTION_BLOCKING_NO_LEAVES, forwards).getY() + 10 + random.nextInt(10);
            }
        ...
        public BlockPos getIslandPos(BlockPos orbit) {
            ...
            int height = level.getHeightmapPos(Heightmap.Type.MOTION_BLOCKING_NO_LEAVES, orbit).getY();
            if (height < 3) {
                island = false;
    ...
    class TemptHeartGoal extends Goal {
        ...
        private final EntityPredicate ENTITY_PREDICATE = (new EntityPredicate()).range(64D).allowInvulnerable().allowSameTeam().allowNonAttackable();

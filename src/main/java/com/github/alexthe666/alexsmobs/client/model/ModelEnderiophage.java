...
public class ModelEnderiophage extends AdvancedEntityModel<EntityEnderiophage> {
    ...
    public void setupAnim(EntityEnderiophage entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        float phagePitch = (float) Math.toRadians(MathHelper.rotLerp(partialTicks, entity.prevPhagePitch, entity.getPhagePitch()));
        float totalYaw = (float) Math.toRadians(MathHelper.rotLerp(partialTicks, entity.yBodyRotO, entity.yBodyRot));
        ...
        if (look != null) {
            ...
            this.eye.yRot += -(float) (MathHelper.atan2(vector3d2.x, vector3d2.z)) - totalYaw;
            this.eye.xRot += -MathHelper.clamp(vector3d2.y * 0.5F, Math.PI * -0.5F, Math.PI * 0.5F) + phagePitch * flyProgress * 0.2F;
        }
    }
